...
public class FileService {
    ...
    public List<Participant> loadParticipants(String path) throws IOException {
        ...
        try (BufferedReader br = Files.newBufferedReader(p)) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                } catch (Exception ex) { AppLogger.warning("Skipping bad participant line " + ln); }
            }
        }
        return participants;
    }
    ...
    public List<Team> loadTeams(String path, List<Participant> participants) throws IOException {
        ...
        try (BufferedReader br = Files.newBufferedReader(p)) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (t == null) {
                    t = new Team(); map.put(teamId, t); teams.add(t);
                }
    ...
    public void saveParticipants(List<Participant> participants, String path) throws IOException {
        ...
        try (BufferedWriter bw = Files.newBufferedWriter(p)) {
            bw.write(PARTICIPANT_HEADER); bw.newLine();
            for (Participant pt : participants) {
                ...
                bw.write(joinEscaped(cols)); bw.newLine();
            }
        }
    }
    ...
    public void saveTeams(List<Team> teams, String path) throws IOException {
        Path p = Path.of(path);
        try (BufferedWriter bw = Files.newBufferedWriter(p)) {
            bw.write(TEAM_HEADER); bw.newLine();
            for (Team t : teams) {
    ...
    private static String[] splitCsvLine(String line) {
        ...
        for (int i = 0; i < line.length(); i++) {
            ...
            } else if (c == ',' && !inQuotes) { cols.add(cur.toString()); cur.setLength(0); }
            else cur.append(c);
        }
